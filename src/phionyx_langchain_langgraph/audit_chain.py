...
# ---------------------------------------------------------------------------
# Signer protocol. get_signer() selects by env: an unconfigured run emits explicitly UNSIGNED
# envelopes (no silent demo signature); demo-HMAC and Ed25519 are opt-in (see get_signer).
# ---------------------------------------------------------------------------
...
# Three signer modes selected by env (get_signer), never silently defaulting to the demo secret
# in a real run:
#   PHIONYX_LANGCHAIN_SIGNING_KEY set -> Ed25519Signer  (real asymmetric signature)
#   PHIONYX_LANGCHAIN_DEMO=1          -> HmacSigner      (E0; the secret ships in-package)
#   neither                           -> UnsignedSigner  (alg='unsigned'; no signature performed)
# A signature names its algorithm by prefix: 'ed25519:<hex>', 'hmac-sha256:<hex>', or 'unsigned'.
...
class Ed25519Signer:
    ...
    def __init__(self, private_key_hex: str, key_id: str = "phionyx-langchain-ed25519") -> None:
...
def get_signer() -> Signer:
    ...
    ``PHIONYX_LANGCHAIN_SIGNING_KEY`` (path or hex) -> Ed25519Signer (key_id via
    ``PHIONYX_LANGCHAIN_KEY_ID``). Else ``PHIONYX_LANGCHAIN_DEMO=1`` -> HmacSigner (E0). Else ->
    UnsignedSigner. A real run with no key provisioned therefore emits explicitly UNSIGNED
    envelopes rather than demo-signed ones that look real."""
    import os

    key = os.environ.get("PHIONYX_LANGCHAIN_SIGNING_KEY")
    if key:
        return Ed25519Signer(_load_key_material(key),
                             key_id=os.environ.get("PHIONYX_LANGCHAIN_KEY_ID", "phionyx-langchain-ed25519"))
    if os.environ.get("PHIONYX_LANGCHAIN_DEMO") == "1":
        return HmacSigner()
...
class EnvelopeContext:
    """All inputs needed to build one envelope (signed by the given signer) from a LangChain event."""
